...
static Eigen::Matrix4d exphat(Eigen::Matrix<double, 6, 1> V) {
    ...
    if (theta < 2e-8) {
        ...
        tempM << eye + sin(theta) * up_hat(omega) + (1 - cos(theta)) * (up_hat(omega) * up_hat(omega));

        Eigen::Vector3d tempV;
        tempV << (eye * theta + (1 - cos(theta)) * up_hat(omega) + (theta - sin(theta)) * up_hat(omega) * up_hat(omega)) * v;
        M << tempM, tempV,
            0, 0, 0, 1;
    }
